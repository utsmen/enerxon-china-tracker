...
def api_db_export(table_name):
    """Temporary: export table as JSON for cross-region migration."""
    import re, base64
    ...
    for r in rows:
        ...
        for k, v in r.items():
            if isinstance(v, (bytes, bytearray, memoryview)):
                row[k] = {'__b64__': base64.b64encode(bytes(v)).decode()}
...
@login_required
def api_db_import():
    """Temporary: import table data from JSON."""
    import base64
    d = request.get_json()
    tbl = d['table']
    rows = d['rows']
...
@login_required
def api_db_tables():
    """Temporary: list tables with counts."""
